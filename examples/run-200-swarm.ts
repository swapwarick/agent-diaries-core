...
async function runSwarm(
  ...
  if (successful === 1)
    console.log(`   🟢 PASSED (${numAgents - 1} race conditions prevented)`);
  ...
  const distributedTasks = Array.from(
    { length: numAgents },
    (_, i) => `${REAL_WORLD_TASKS[i % 10]} - Run ${runToken}`,
  );

  startTime = Date.now();
  results = await Promise.all(
    agents.map((agent, i) =>
      agent.claimTask(distributedTasks[i]).catch((e) => false),
    ),
  );
  ...
  if (successful === 10)
    console.log(
      `   🟢 PASSED (${numAgents - 10} duplicate LLM calls prevented)`,
    );
  else console.log(`   🔴 FAILED`);
  ...
  const uniqueTasks = Array.from(
    { length: numAgents },
    (_, i) => `Independent Research Topic ${i} - ${runToken2}`,
  );
  ...
  await Promise.all(
    agents.map((agent, i) =>
      agent.writeTaskResult(
        uniqueTasks[i],
        `{"status": "completed", "tokens": 4050, "data": "..."}`,
      ),
    ),
  );
  duration = Date.now() - startTime;
  ...
  if (testWrites.length === numAgents)
    console.log(`   🟢 PASSED (Zero data corruption)`);
  else console.log(`   🔴 FAILED`);
}
...
async function main() {
  ...
  if (process.env.UPSTASH_REDIS_URL) {
    ...
    await runSwarm(
      ...
      55,
      () =>
        new AgentDiary({ agentId: "cloud-swarm-bot", storage: redisStorage }),
    );
  ...
  if (process.env.MONGO_URI) {
    ...
    await runSwarm(
      "MongoDB (Atomic Upserts)",
      200,
      () => new AgentDiary({ agentId: "swarm-bot", storage: mongoStorage }),
    );

...
async function verifyStorage(name: string, storage: any, client: any) {
  ...
  try {
    ...
    if (e.message.includes("was not claimed")) {
      console.log(
        `   🟢 PASSED: Successfully blocked silent insert! Error caught: "${e.message}"`,
      );
    } else {
  ...
  console.log(
    `[Test 2] Verifying subsequent claims run fast (Static Cache)...`,
  );
  ...
  console.log(
    `   🟢 PASSED: 3 successive locks acquired in ${duration}ms (No cold-start penalty on 2nd/3rd lock)`,
  );

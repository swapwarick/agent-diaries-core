...
describe('SqliteStorage Adapter & AgentDiary Integration', () => {
  ...
  it('should prevent silent inserts via strict ownership (AgentDiary integration)', async () => {
    ...
    await expect(diary.writeTaskResult('Unclaimed Task', 'Success Result'))
      .rejects
  ...
  it('should recover from expired locks', async () => {
    ...
    db.prepare('INSERT INTO agent_diaries_locks (key, locked_at) VALUES (?, ?)')
    ...
    const lock = db.prepare('SELECT locked_at FROM agent_diaries_locks WHERE key = ?')
      .get(lockKey);
